...
class Trainer(object):
    ...
    def val_epoch(self, epoch, val_dataloader, i=2):
        ...
        print(f'Val Epoch {epoch}: average Loss: {val_loss:.6f}')  # Print to console
        return val_loss
    ...
    def test_epoch(self, epoch, test_dataloader, i=2):
        ...
        print(f'Test Epoch {epoch}: average Loss: {test_loss:.6f}')  # Print to console
        return test_loss
    ...
    def train_epoch(self, epoch, i=2):
        ...
        print(f'Train Epoch {epoch}: averaged Loss: {train_epoch_loss:.6f}')  # Print to console
    ...
    def train(self):
        ...
        self.logger.info("Starting first phase of training")
        print("Starting first phase of training")  # Print to console
        ...
        for epoch in range(1, 0):
            ...
            if train_epoch_loss > 1e6:
                ...
                print('Gradient explosion detected. Ending...')  # Print to console
                break
            #if self.val_loader == None:
            ...
            if self.args.early_stop:
                if not_improved_count == self.args.early_stop_patience:
                    ...
                    print(
                        f"Validation performance didn't improve for {self.args.early_stop_patience} epochs. Training stops.")  # Print to console
                    break
            # save the best state
            if best_state == True:
    ...
    def test(model, args, data_loader, scaler, logger, path=None):
        ...
        # 将y_true保存为csv
        # y_true_df = pd.DataFrame(y_true.cpu().numpy())
        # y_true_df.to_csv('./{}_true.csv'.format(args.dataset), index=False)
        #
        # # 将y_pred保存为csv
        # y_pred_df = pd.DataFrame(y_pred.cpu().numpy())
        # y_pred_df.to_csv('./{}_pred.csv'.format(args.dataset), index=False)
        y12 = y_true[:, 99, 11].cpu().detach().numpy()
        yhat12 = scaler.inverse_transform(y_pred[:, 99, 11]).cpu().detach().numpy()

        y3 = y_true[:, 99, 2].cpu().detach().numpy()
        yhat3 = scaler.inverse_transform(y_pred[:, 99, 2]).cpu().detach().numpy()

        df2 = pd.DataFrame({'real12': y12, 'pred12': yhat12, 'real3': y3, 'pred3': yhat3})
        df2.to_csv('./{}_wave.csv'.format(args.dataset), index=False)
